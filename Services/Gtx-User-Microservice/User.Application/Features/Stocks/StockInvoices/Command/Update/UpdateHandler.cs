...
namespace User.Application.Features.Stocks.StockInvoices.Command.Update
{
    public class UpdateHandler : IRequestHandler<UpdateCommand, UpdateVm>
    {
        ...
        public UpdateHandler(IStockInvoiceRepository repository, IMapper mapper, IOptions<ImageServer> imageServer)
        {
        ...
        public async Task<UpdateVm> Handle(UpdateCommand request, CancellationToken cancellationToken)
        {
            ...
            var isExits = await _repository.GetAsync(s => s.Id != entity.Id && s.BillNumber.ToLower() == entity.BillNumber.ToLower());
